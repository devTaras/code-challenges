    ...
    class GenomicRangeQuery
    {
        public int[] solution(string S, int[] P, int[] Q)
        {
            ...
            for (int i = 0; i < S.Length; i++)
            {
                ...
                switch (S[i])
                {
                    case 'A': matrix[i, 0] = i; break;
                    case 'C': matrix[i, 1] = i; break;
                    case 'G': matrix[i, 2] = i; break;
                    case 'T': matrix[i, 3] = i; break;
                }
                if (i > 0)
                {
                    matrix[i, 0] = Math.Max(matrix[i, 0], matrix[i - 1, 0]);
                    matrix[i, 1] = Math.Max(matrix[i, 1], matrix[i - 1, 1]);
                    matrix[i, 2] = Math.Max(matrix[i, 2], matrix[i - 1, 2]);
                    matrix[i, 3] = Math.Max(matrix[i, 3], matrix[i - 1, 3]);
                }
            }
            ...
            for (int i = 0; i < P.Length; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    if (matrix[Q[i], j] >= P[i]) { res = j + 1; break; }
                }
                result[i] = res;
            }
